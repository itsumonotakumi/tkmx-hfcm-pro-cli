...
# サンプル設定をコピーして編集（INI 形式推奨）
cp tkmx-hfcm-pro-cli/config/cli.sample.ini tkmx-hfcm-pro-cli/config/cli.local.ini
chmod 0600 tkmx-hfcm-pro-cli/config/cli.local.ini
# cli.local.ini を編集して HFCM_CLI_DEFAULT_USER に管理者のログイン名を設定
```
...
Bootstrap は以下の優先順位で設定ファイルを読み込みます:

1. `config/cli.local.ini`（**推奨**）— PHP コード実行なし、副作用ゼロの宣言的設定
2. `config/cli.local.php`（**非推奨**）— 後方互換のため維持。INI ファイルが存在する場合は読み込まれません

いずれのファイルも:
- 実行ユーザーに所有されている必要があります
- パーミッションがちょうど `0600` である必要があります
- シンボリックリンクは拒否されます

### INI 形式（推奨）

`config/cli.local.ini` を編集：

```ini
; 必須: manage_options 権限を持つ WP ユーザー
HFCM_CLI_DEFAULT_USER = "admin"

; オプション: --as による偽装を許可
; HFCM_CLI_ALLOW_AS = "1"

; オプション: 偽装を許可するユーザーのリスト
; HFCM_CLI_ALLOWED_AS_USERS[] = "admin"
; HFCM_CLI_ALLOWED_AS_USERS[] = "editor"
```
...
**セキュリティ**: `config/cli.local.ini` は実行ユーザーに所有され、パーミッションが `0600` である必要があります。  
異なる権限の場合は exit 4 (FORBIDDEN) で終了します。

### INI 移行ガイド

既存の `cli.local.php` から `cli.local.ini` へ移行する手順:

```bash
# 1. INI サンプルをコピー
cp config/cli.sample.ini config/cli.local.ini
chmod 0600 config/cli.local.ini

# 2. cli.local.php の設定を cli.local.ini に転記
#    例: define('HFCM_CLI_DEFAULT_USER', 'admin');
#    → HFCM_CLI_DEFAULT_USER = "admin"

# 3. 動作確認後、旧ファイルを削除（任意）
# rm config/cli.local.php
```

| PHP 形式 | INI 形式 |
|---------|---------|
| `define('HFCM_CLI_DEFAULT_USER', 'admin');` | `HFCM_CLI_DEFAULT_USER = "admin"` |
| `define('HFCM_CLI_ALLOW_AS', '1');` | `HFCM_CLI_ALLOW_AS = "1"` |
| `define('HFCM_CLI_ALLOWED_AS_USERS', ['admin', 'editor']);` | `HFCM_CLI_ALLOWED_AS_USERS[] = "admin"` + `HFCM_CLI_ALLOWED_AS_USERS[] = "editor"` |

### Deprecation スケジュール

| フェーズ | 内容 |
|--------|------|
| 現在（このバージョン） | `cli.local.ini` 優先読み込み。`cli.local.php` は動作するが STDERR に deprecation warning を出力 |
| 次期メジャーバージョン | `cli.local.php` サポート削除予定 |
...
**`Error: config/cli.local.ini の権限は 0600 である必要があります`**  
以下を実行: `chmod 0600 config/cli.local.ini`

**`Error: config/cli.local.ini は現在のユーザーに所有される必要があります`**  
ファイルが CLI 実行ユーザーに所有されていることを確認: `chown $(whoami) config/cli.local.ini`

**`Error: config/cli.local.php must have mode 0600`**（PHP 形式利用時）  
以下を実行: `chmod 0600 config/cli.local.php`

**`Error: config/cli.local.php must be owned by the current user`**（PHP 形式利用時）  
